...
// db.serialize(() => {
//     db.run("CREATE TABLE requests (request_id TEXT, status TEXT, created_at TEXT, completed_at TEXT)");
//     db.run("CREATE TABLE images (image_id INTEGER PRIMARY KEY, request_id TEXT, product_name TEXT, input_url TEXT, output_url TEXT, status TEXT)");
// });
...
//     db.run("UPDATE requests SET status = ?, completed_at = ? WHERE request_id = ?", [status, completedAt, requestId]);
// }

// function addImage(requestId, productName, inputUrl, outputUrl, status) {
//     db.run("INSERT INTO images (request_id, product_name, input_url, output_url, status) VALUES (?, ?, ?, ?, ?)", [requestId, productName, inputUrl, outputUrl, status]);
// }
...
// module.exports = { addRequest, updateRequestStatus, addImage, getRequestStatus };
